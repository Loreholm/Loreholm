...
EMBEDDING_DIMENSIONS="${LOREHOLM_V2_EMBEDDING_DIMENSIONS:-384}"
...
if [[ -e "$ENV_FILE" ]]; then
  ...
  umask 077
  {
    ...
    printf 'LOREHOLM_V2_EMBEDDING_DIMENSIONS=%s\n' "$EMBEDDING_DIMENSIONS"
...
if ! grep -q '^LOREHOLM_V2_EMBEDDING_DIMENSIONS=' "$ENV_FILE"; then
  previous_umask="$(umask)"
  umask 077
  printf 'LOREHOLM_V2_EMBEDDING_DIMENSIONS=%s\n' \
    "$EMBEDDING_DIMENSIONS" >> "$ENV_FILE"
  umask "$previous_umask"
fi
