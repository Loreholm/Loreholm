...
# Usage: irm __APP_DOMAIN__/install.ps1 | iex
#        Or: Invoke-WebRequest -Uri https://__APP_DOMAIN__/install.ps1 -OutFile install.ps1; .\install.ps1 -Key "your-key"
#
param(
...
Required:
  -Key <key>            Pre-authentication key from __APP_DOMAIN__ dashboard
...
For one-line web install:
  irm __APP_DOMAIN__/install.ps1 | iex
...
function Show-Status {
    ...
    Write-Host "  └─ Uninstall:     irm __APP_DOMAIN__/uninstall.ps1 | iex"
    Write-Host ""
    ...
    Write-Host "  3. Return to https://__APP_DOMAIN__/dashboard to verify connection"
...
function Invoke-Main {
    ...
    if (-not $Key -and -not $PSBoundParameters.ContainsKey('Key')) {
        ...
        Write-Host ""
        Write-Host "Get your key from: https://__APP_DOMAIN__/dashboard" -ForegroundColor $Colors.Blue
        Write-Host ""
    ...
    if (-not $Key) {
        if (Test-Path (Join-Path $InstallDir "docker-compose.yml")) {
            Write-Error-Custom "Pre-auth key is required for first-time install. Existing installation detected at $InstallDir. Run: irm __APP_DOMAIN__/update.ps1 | iex"
        }
