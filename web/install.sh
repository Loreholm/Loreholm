...
# Usage: curl -fsSL __APP_DOMAIN__/install.sh | bash -s -- --key <pre-auth-key>
# 
# For Windows, use: irm __APP_DOMAIN__/install.ps1 | iex
#
set -euo pipefail
...
show_banner() {
    echo -e "${BLUE}"
    cat << 'EOF'
    __    ____  ____  ________  ______  __    __  ___
   / /   / __ \/ __ \/ ____/ / / / __ \/ /   /  |/  /
  / /   / / / / /_/ / __/ / /_/ / / / / /   / /|_/ / 
 / /___/ /_/ / _, _/ /___/ __  / /_/ / /___/ /  / /  
/_____/\____/_/ |_/_____/_/ /_/\____/_____/_/  /_/   
                                                     

    Your data Lives where you do
EOF
    echo -e "${NC}"
}
...
parse_args() {
    ...
    if [[ -z "$PRE_AUTH_KEY" ]]; then
        if [[ -f "$COMPOSE_FILE" ]]; then
            error "Pre-auth key is required for first-time install. Existing installation detected at $INSTALL_DIR. Run: curl -fsSL https://__APP_DOMAIN__/update.sh | bash"
        fi
...
show_status() {
    ...
    echo "  └─ Uninstall:     curl -fsSL __APP_DOMAIN__/uninstall.sh | bash"
    echo ""
    ...
    echo "  3. Return to https://__APP_DOMAIN__/dashboard to verify connection"
